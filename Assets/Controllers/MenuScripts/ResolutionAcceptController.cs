...
public class ResolutionAcceptController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Screen.width != initialResolution.x || Screen.height != initialResolution.y)
        {
            UpdateFontSizes();
            SetResolutionToMatchDevice();
            UpdateRawImageResolution();
    ...
    private void UpdateFontSizes()
    {
        ...
        foreach (TMP_Text tmpText in tmpTexts)
        {
            if (tmpText != null)
            {
                ...
                float newFontSize = initialFontSize * averageScaleFactor - 16;
    ...
    {
        Resolution deviceResolution = Screen.currentResolution;
        if (deviceResolution.width < 1300 || deviceResolution.height < 720) {
            lowres = true;
        }
        ...
        if (lowres == true) sizeMultiplier = 1.20f;
